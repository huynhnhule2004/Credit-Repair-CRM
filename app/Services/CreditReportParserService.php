...
class CreditReportParserService
{
    ...
    {
        try {
            // Detect report format early to guide parser selection
            $textPreview = '';
            try {
                ...
                Log::warning("Format detection could not read PDF text: {$t->getMessage()}");
                $text = '';
            }

            $format = $this->detectReportFormat($text ?? '');
            Log::info("Detected credit report format: {$format}");

            // Use IdentityIqFullParser for PDF parsing
            $fullParser = new IdentityIqFullParser();
            $result = $fullParser->parseAndSave($client, $pdfPath, $format);
    ...
    /**
     * Lightweight format detection to prioritize parsing paths.
     */
    private function detectReportFormat(string $text): string
    {
        $t = strtolower($text);

        $hasDashboard = str_contains($t, 'credit score dashboard');
        $has3Bureaus = preg_match('/transunion|experian|equifax/i', $text) === 1;
        $hasPerBureau = preg_match('/(experian|equifax|transunion)\s+credit\s+file/i', $text) === 1;

        if ($hasDashboard && $has3Bureaus) {
            return 'combined_3bureau';
        }
